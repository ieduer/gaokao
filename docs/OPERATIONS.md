## 2026-09-25: exact PR11 capture release accepted

Production runtime source `0b3258ffcc755f4dc2939b3e2e22bf43f56519d4` on main, Pages `206bb0dc-42d4-4c5c-bed8-d421aa7b6572` on gk.bdfz.net. Existing native guarded publisher merged/published the exact requested commit.10public artifact hashes, source/tree/policy and preserved Function inputs match; Function readback passes. Two fresh system/record_only operations preserve full content/context/time/digest and ordinary authenticated reload, one row each. Provider calls0, grade writes0, notifications0. First immediate central read raced durable synchronization; only read-only completion followed, with no new fixture or resend. Rollback `ba438f25-c986-44e3-aefe-f5bd063ec952` / source `897ca30111e9d96adcaf7bf1aba95c1f2e936a00`, preserving all forward data. Canonical unrelated dirty work untouched. Detailed authority: /Users/ylsuen/CF/reports/operations/learning-records-validity-20260925/GK_RELEASE.md. Documentation branch is separate and does not change the exact runtime source. Server-reply-loss recovery, native BFCache and natural-flow effectiveness remain unproven for GK; remaining leaves/report work stays open.
...
The accepted UC recorder is reused byte-for-byte. Explicit original drafts/submissions, question exposure, full AI prompts/replies and error content append to private record_only observations. Unknown identity stays unowned; original context/time is fixed before asynchronous work. Original submissions and request context must persist before AI. Existing generic progress/scoring, content and Function behavior are unchanged. Server HTTP reply loss remains a separate unimplemented recovery boundary.

Prepublication:18 tests pass,612questions/472annotations and Beijing2026 checks pass without content changes. Real authenticated synthetic source/central/reload, duplicate/replay and offline reload recovery passed with two system/record_only rows. A first offline check saw an empty source inbox because its row had moved to the second durable outbox; the corrected check verified exact scope/content/time in that outbox across offline reload and exact one-row central readback afterward. No failure was hidden or relabeled. Browser evidence and source hashes: gk-browser-candidate.json in the same evidence root. Native bfcache entry is unproven; the persisted-pageshow recovery event was injected.

After automatic publication, require exact live source/artifact hashes, preserved Function and ordinary production reload/readback before acceptance. Rollback must name the exact baseline deployment above and retain forward records. No provider calls, grades, notifications, new runtime capability or credentials are part of this leaf release. Runtime artifacts remain in the shared task manifest; source/candidate acceptance does not authorize deleting prior evidence or another owner's dirty canonical documents.
